...
SLACK_WEBHOOK_URL  = os.environ.get('SLACK_WEBHOOK_URL', '')  # Set in Lambda env vars
    ...
    try:
        ...
        if maintenance_services:
            ...
            if detected_service and detected_service in maintenance_services:
                ...
                # NEW: Slack alert for maintenance mode
                send_slack_alert(
                    analysis={
                        "issue":              f"{detected_service.capitalize()} stopped during planned maintenance",
                        "severity":           "LOW",
...
# ═════════════════════════════════════════════
# NEW: SLACK INCIDENT ALERT
# ═════════════════════════════════════════════

def send_slack_alert(analysis, remediation_result, instance_id, log_group):
    """
    Post a formatted incident alert to a Slack channel via Incoming Webhook.
    Uses Slack Block Kit for rich formatting with severity color coding.
    ...
    Skipped silently if SLACK_WEBHOOK_URL is not configured.
    Never raises — alert failure must not crash the main pipeline.
    """
                ...
                "blocks": [
                    ...
                    },
                    {
                        ...
                        "elements": [
                            {
                                "type": "mrkdwn",
                                "text": f"AIOps Self-Healing Pipeline | EC2 → CloudWatch → Lambda → Bedrock → SSM | {timestamp}"
                            }
                        ]
                    }
                ]
    ...
    try:
        ...
        with urllib.request.urlopen(request, timeout=5) as resp:
            ...
            print(f"💬 Slack alert sent | Status: {resp.status} | Severity: {severity} | Remediation: {rem_status}")
    except Exception as e:
        # Alert failure must never crash the main pipeline
