...
def lambda_handler(event, context):
    ...
    try:
        ...
        # ── Step 5.5: Save to DynamoDB + Publish Metrics (UPDATED) ──
        ...
        # NEW: Publish custom metrics to CloudWatch
...
# ═════════════════════════════════════════════
# NEW: CLOUDWATCH CUSTOM METRICS
# ═════════════════════════════════════════════

def publish_custom_metrics(analysis, remediation_result, instance_id, mttr_ms):
    """
    Publish structured incident metrics to CloudWatch under the
    'AIOps/Incidents' namespace. These metrics power the live
    operations dashboard and can trigger CloudWatch Alarms.

    Metrics published per incident:
      - IncidentCount      : always 1 (counts total incidents)
      - SeverityScore      : CRITICAL=4, HIGH=3, MEDIUM=2, LOW=1
      - MTTRMilliseconds   : end-to-end pipeline duration
      - RemediationSuccess : 1 if success, 0 otherwise
      - RemediationFailed  : 1 if failed, 0 otherwise

    Dimensions allow filtering by instance and service in the dashboard.
    Never raises — metric failure must not crash the pipeline.
    """
    try:
        severity          = analysis.get('severity', 'UNKNOWN')
        remediation_status = remediation_result.get('status', 'unknown')
        service           = analysis.get('remediation_action', 'none')
        service_name      = detect_affected_service(service) or 'unknown'

        # Severity → numeric score for trend graphing
        severity_score = {
            'CRITICAL': 4,
            'HIGH':     3,
            'MEDIUM':   2,
        ...
        # Remediation outcome → binary metrics
        is_success = 1 if remediation_status == 'success'  else 0
        is_failed  = 1 if remediation_status == 'failed'   else 0
        is_skipped = 1 if remediation_status == 'skipped'  else 0

        # Common dimensions — filter by instance or service in dashboard
        dimensions_instance = [
            {'Name': 'InstanceId', 'Value': instance_id or 'unknown'},
        ...
        metric_data = [
            ...
            # Severity as numeric score (for trend line)
            {
            ...
            # Incident count broken down by severity
            {
                'MetricName': 'IncidentBySeverity',
                'Dimensions': dimensions_severity,
                'Value':      1,
                'Unit':       'Count'
            },
            # MTTR in milliseconds
            {
                'MetricName': 'MTTRMilliseconds',
                'Dimensions': dimensions_instance,
                'Value':      float(mttr_ms),
                'Unit':       'Milliseconds'
            },
            ...
            {
                'MetricName': 'RemediationFailed',
                'Dimensions': dimensions_instance,
                'Value':      is_failed,
                'Unit':       'Count'
            },
            {
                'MetricName': 'RemediationSkipped',
                'Dimensions': dimensions_instance,
                'Value':      is_skipped,
                'Unit':       'Count'
            },
        ...
        cloudwatch.put_metric_data(
            Namespace='AIOps/Incidents',
            MetricData=metric_data
        )
        print(f"📊 Metrics published | Severity: {severity} ({severity_score}) | MTTR: {mttr_ms}ms | Remediation: {remediation_status}")

    except Exception as e:
        # Metric failure must never crash the main pipeline
        print(f"⚠️  Failed to publish CloudWatch metrics: {str(e)}")
